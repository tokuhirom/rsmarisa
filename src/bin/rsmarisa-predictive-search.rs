//! rsmarisa-predictive-search - Find keys with given prefixes in a MARISA trie
//!
//! For each query, finds all keys that start with the query.
...
#[derive(Parser)]
#[command(name = "rsmarisa-predictive-search")]
#[command(about = "Find keys that start with given prefixes")]
#[command(version)]
struct Args {
